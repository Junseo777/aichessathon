...
Generally, an unvisited move is reachable only when

```
P > Q_explored / (c_puct * sqrt(N))
```

which here is `0.79 / (1.5 * 20) ~= 0.026`. Every move with a prior under ~2.6%
is **arithmetically excluded**, not merely unlikely.
...
and everything gets tried; at `Q ~= 0.8` a quarter of the move list becomes
invisible. The agent stops looking for wins exactly when it is winning.
